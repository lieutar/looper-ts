...
import { AfterInit, Delegate, Init, LazyInit, qoop, } from 'qoop';
import { isFile } from 'looper-utils';
...
import { canonicalizeDirectoryPath } from 'looper-utils';
import type { Logger } from '../../../../utils/fancy-logger/src/Logger';
...
export const defaultDirectoryIndex = ['index', 'index.html', 'index.htm', 'index.text', 'index.txt'];
...
export class FsRouter {
  ...
  @Delegate('app', {get:true}) declare readonly logger: Logger;
  @Init() declare documentRoot: string;
  @Init({default: '.action.ts'}) declare actionSuffix: string;
  @Init({makeDefault: ()=>[... defaultDirectoryIndex]} ) declare  directoryIndex: string[];
  @Init((params:any)=>params.suffixRules ?? [... defaultSuffixRules]) suffixRules!: SuffixRuleType[];

  constructor(_params: FsRouterParams){}
  @AfterInit() protected _init(){ this.app.registerService(DFsRouter, this); }
